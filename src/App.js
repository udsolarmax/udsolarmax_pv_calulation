...
export default function App() {
  ...
  // --- 2. Advanced Settings (ละเอียดขึ้น) ---
  const [railUnitLen, setRailUnitLen] = useState(4.20); // ความยาวรางมาตรฐานต่อเส้น (เมตร)
  const [midClamp, setMidClamp] = useState(20);    // มม.
  const [endClamp, setEndClamp] = useState(40);    // มม. (เฉพาะตัว Clamp)
  const [overhang, setOverhang] = useState(50);    // มม. (ระยะเผื่อปลายราง)
  const [lFeetDist, setLFeetDist] = useState(1200); // มม.

  // --- 3. สูตรคำนวณ (Calculation Logic) ---
  
  // A. ความยาวเชิงเรขาคณิต (Total Geometry Length)
  // สูตร: (PanelZone) + (MidGaps) + (EndClamps) + (Overhangs)
  const totalPanelWidth = panels * panelWidth;
  const totalMidGaps = (panels > 0 ? panels - 1 : 0) * (midClamp / 1000);
  const totalEndClamps = 2 * (endClamp / 1000);
  const totalOverhangs = 2 * (overhang / 1000);

  const railLengthPerString = totalPanelWidth + totalMidGaps + totalEndClamps + totalOverhangs;

  // B. คำนวณวัสดุ (BOM)
  const totalRailLines = 2 * strings; // จำนวนแนวรางทั้งหมด
  
  // B1. จำนวนรางที่ต้องสั่ง (เส้น)
  // เอาความยาวที่ต้องใช้ หารด้วย ความยาวรางมาตรฐาน แล้วปัดขึ้น
  const barsPerLine = Math.ceil(railLengthPerString / railUnitLen); 
  const totalBars = barsPerLine * totalRailLines;

  // B2. ตัวต่อราง (Splice)
  // ถ้าต้องใช้ราง 2 เส้นต่อกัน ต้องใช้ Splice 1 ตัว (Bars - 1)
      ...
      <div className="bg-white p-4 md:p-6 rounded-xl shadow-lg w-full max-w-6xl border-t-8 border-[#0ea5e9]">
        <div className="flex flex-col md:flex-row justify-between items-center mb-6">
          <h1 className="text-xl md:text-2xl font-bold text-gray-800">
            UD Solarmax <span className="block md:inline text-xs md:text-sm font-normal text-gray-500">Engineering Calc v6.3 (Production)</span>
          </h1>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
          ...
          <div className="lg:col-span-3 space-y-4">
            ...
            <div className="bg-yellow-50 p-3 rounded-lg border border-yellow-200 space-y-2">
             ...
             {/* Inputs Group 3: Distances (Updated) */}
          ...
          <div className="lg:col-span-3 space-y-4">
                ...
                <table className="w-full text-xs text-left">
                   <tbody className="divide-y divide-gray-100">
                      ...
                      <tr className="bg-yellow-50">
                        <td className="px-3 py-2 text-yellow-800 font-bold">จำนวนราง ({railUnitLen}ม.)</td>
             ...
             <div className="p-3 bg-blue-50 rounded-lg text-[10px] text-blue-800 border border-blue-100">
               ...
               ความยาว = (แผง x {panels}) + MidGap + EndClamp + เผื่อปลาย<br/>
               จำนวนเส้น = ปัดเศษขึ้น (ความยาว / {railUnitLen})
             </div>
          </div>
